...
- (void)handleXHRMessage:(WKScriptMessage *)message
{
    ...
    if ([postData isKindOfClass:[NSString class]]) {
        ...
        bodyStream = CFBridgingRelease(readStream);
    ...
    if ([self isRelativeUrl:url]) {
        ...
        [self sendHttpRequest:reqID method:method url:url body:bodyStream headers:mutableHeaders];
    }
}
...
- (void)sendHttpRequest:(NSNumber *)requestId method:(NSString *)requestMethod url:(NSString *)url body:(NSInputStream *)dataStream headers:(NSDictionary *)headers
{
